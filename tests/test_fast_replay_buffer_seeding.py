...
from utils.rl_utils import import_training_into_replay_buffer_v3
from stable_baselines3 import SAC as SB3_SAC
from utils.test_utils import binary_compare
...
def test_fast_replay_buffer_seeding(flag_report_live: bool = False):
    ...
    test_log = []
    test_log = log(
        "Test Fast Replay Buffer Seeding", test_log, flag_report_live
    )

    # buffer size
    buffer_size = int(params.get("input_buffer_size", 100000))

    test_log = log("Importing ephems from: " + str(params["path_ephems"]), test_log, flag_report_live)
    test_log = log("Number of ephems to use: " + str(params["num_ephems_to_use"]), test_log, flag_report_live)
    test_log = log("Input buffer capacity: " + str(buffer_size), test_log, flag_report_live)


    # read the ephems from directory
    set_ephems, filenames = read_ephems_from_dir(  params["path_ephems"], 
                                                  params["num_ephems_to_use"],  
                                                  version=params["ephem_version"], 
                                                  flag_return_filenames=True,
                                                  params=params )
    
    test_log = log(f"Number of ephems read: {len(set_ephems)}", test_log, flag_report_live)
    ...
    test_log = log("Starting size: " + str(model.replay_buffer.size()), test_log, flag_report_live)

    import_training_into_replay_buffer_v3( set_ephems, test_log, model, env, params )

    path_replay_buffer = os.path.join(path_test, "test_fast_replay_buffer_seeding_replay_buffer.pkl")
    model.save_replay_buffer(path_replay_buffer)

    test_log = log("Buffer capacity: " + str(buffer_size), test_log, flag_report_live)
    test_log = log("Ending buffer size: " + str(model.replay_buffer.size()), test_log, flag_report_live)

    # compare to truth
    path_replay_buffer_truth = os.path.join(path_test, "truth_fast_replay_buffer_seeding_replay_buffer.pkl")

    flag_pass = binary_compare(path_replay_buffer, path_replay_buffer_truth);
    test_log = log(f"Comparing generated replay buffer to truth buffer.", test_log, flag_report_live)
    test_log = log(f"Generated replay buffer path: {path_replay_buffer}", test_log, flag_report_live)
    test_log = log(f"Truth replay buffer path: {path_replay_buffer_truth}", test_log, flag_report_live)

    if flag_pass:
        test_log = log("Fast replay buffer seeding test PASSED.", test_log, flag_report_live)
    else:
        test_log = log("Fast replay buffer seeding test FAILED.", test_log, flag_report_live)
