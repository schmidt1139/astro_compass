...
def main():
    os.makedirs(os.path.join(PLOT_ROOT, "tests"), exist_ok=True)
    ...
    arr_y_nd, g0_nd, mu_nd, T_max_nd, ISP_nd, input_TOF_nd = non_dimensionalize(
        arr_y0, g0, mu, T_max, ISP, input_TOF, l_star, m_star, t_star
    )
    ...
    arr_time = sol.t
    variables = sol.y
    arr_time_yrs = arr_time * t_star / years_in_s
    ...
    E0 = (
        ...
        / 2
        * (
            (variables[2, 0] * l_star / t_star) ** 2
            + (variables[3, 0] * l_star / t_star) ** 2
        )
        - mu / r_0
    )
    ...
    for index, t in enumerate(arr_time):
        ...
        eph.add_data(t, x_i, y_i, vx_i, vy_i, m_i, alpha_vec[0], alpha_vec[1], u)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(PLOT_ROOT, "tests", "states_nd.pdf"))  # Vector format
    plot.show()
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(PLOT_ROOT, "tests", "costates_nd.pdf"))  # Vector format
    plot.show()

    fig, ax = plot.subplots(figsize=(6, 6))
    ax.plot(arr_time_yrs, arr_E)
    ax.plot(arr_time_yrs, arr_E0)
    ax.set_title("Orbit Energy over Time")

    fig, ax = plot.subplots(figsize=(6, 6))
    ax.plot(arr_time_yrs, arr_alpha_x, label="alpha_x")
    ax.plot(arr_time_yrs, arr_alpha_y, label="alpha_y")
    ax.set_title("Alpha Vector (Maneuver Direction) over Time")
    ax.legend()

    fig, ax = plot.subplots(figsize=(6, 6))
    ax.plot(arr_time_yrs, arr_m)
    ax.set_title("Spacecraft Mass Fraction over Time")

    fig, ax = plot.subplots(figsize=(6, 6))
    ax.plot(arr_time_yrs, arr_u)
    ax.set_title("Spacecraft Thrust Throttle over Time")

    fig, ax = plot.subplots(figsize=(6, 6))
    ax.plot(arr_time_yrs, arr_rho)
