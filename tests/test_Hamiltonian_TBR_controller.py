...
def test_Hamiltonian_TBR_Controller(flag_report_live):
    ...
    params = {
        ...
        ** 0.5,  # characteristic time - derived
        ...
        "data_path": os.path.join(
            "data", "test_data", "test_TBR_hamiltonian"
    ...
    for item in arr_pass_count:
        test_log = log(
            "Traj " + str(count) + " solution found: " + str(item),
            test_log,
            flag_report_live,
        )
        count += 1
    ...
    for output_file in sa_output_ephems:
        ...
        truth_file = os.path.join(
            params["data_path"], f"test_TBR_ephem_traj_seed_{seed_num}_truth.txt"
        )
    ...
    if params["flag_compare_to_truth"] == True:
        ...
        if flag_all_match:
            test_log = log(
                "All trajectories match truth data!", test_log, flag_report_live
            )
        else:
            test_log = log(
