...
# Ensure repository paths are on sys.path for direct test execution
...
if "TwoBody_Orb2Orb_Transfer_Env_nd-v0" not in envs.registry.keys():
    register(
        ...
        entry_point=TwoBody_Orb2Orb_Transfer_Env_nd,
...
def test_SAC_training(flag_report_live=False, seed_in=42):
    ...
    test_log = log(
        "Max steps per episode: " + str(env.spec.max_episode_steps),
        test_log,
        flag_report_live,
    )
