...
def convert_single_ephem(ephem, dir_out, filename_out):
    ...
    for i in range(1, num_vectors):
        ...
        ephem_v3.add_data(
            ...
            vy,
            mass,
            arr_target_current[0],
            arr_target_current[1],
            arr_target_current[2],
            arr_target_current[3],
            ttg,
            alpha_x,
            alpha_y,
            u,
...
def convert_ephems():
    ...
    tasks = [
        (set_ephems[i], path_output, filenames[i].replace(".txt", "_v3.txt"))
        for i in range(num_ephems)
    ]

    with concurrent.futures.ProcessPoolExecutor(max_workers=cores) as executor:
        list(
            tqdm(
                executor.map(convert_ephem_wrapper, tasks),
                total=len(tasks),
                desc="Converting ephemerides",
            )
        )
