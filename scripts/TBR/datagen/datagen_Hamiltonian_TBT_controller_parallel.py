...
from core.datagen import run_parallel_transfer_generation
from utils.log_utils import log, read_config_file, write_log_to_file


def datagen_Hamiltonian_TBT_controller_parallel():
    start_time = time.time()
    time_str = time.strftime("%Y%m%d_%H%M%S")

    plot.style.use(os.path.join("data", "support_files", "light_paper.mplstyle"))

    path_config = os.path.join(
        "data", "config", "datagen_Hamiltonian_TBT_controller_parallel_config.txt"
    )
    ...
    test_log, arr_pass_count, sa_output_ephems, sa_summary = (
        run_parallel_transfer_generation(params)
    )
...
if __name__ == "__main__":
    ...
    profiler.enable()
    datagen_Hamiltonian_TBT_controller_parallel()
    profiler.disable()
