...
def test_Hamiltonians(flag_report_live=False):
    ...
    test_log = log("Initial observation vector: " + str(init_observation), test_log, flag_report_live)
    ...
    H_controller = Hamiltonian_Controller_TBT(env, init_observation, init_info, input_TOF, **kwargs)

    # compute solution
    flag_solved, h_sol, eps, sol, log_hsl = H_controller.hamiltonian_solution_finder()

    test_log = log("Hamiltonian solution found: " + str(flag_solved), test_log, flag_report_live)
    ...
    test_log = log("Solution for initial co-states: " + str(h_sol), test_log, flag_report_live)
    test_log = log("Final smoothing parameter used in solution generation: " + str(eps), test_log, flag_report_live)
    #print(sol)
