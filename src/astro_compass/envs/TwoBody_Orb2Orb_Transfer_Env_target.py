...
from astro_compass.utils.state_vector_utils import (
    ...
    calc_OE_from_cart,
    cartesian_to_polar,
    compute_eccentric_anomaly_from_true_anomaly,
    compute_mean_anomaly_from_eccentric_anomaly,
...
class TwoBody_Orb2Orb_Transfer_Env_target(gym.Env):
    ...
    def _get_info(self, ode_solution, delta_r):
        ...
        self.eccentric_anomaly = compute_eccentric_anomaly_from_true_anomaly(
            self._keplerian_elements[3], self._keplerian_elements[1]
        )

        # compute mean anomaly
        self.mean_anomaly = compute_mean_anomaly_from_eccentric_anomaly(
            self.eccentric_anomaly, self._keplerian_elements[1]
        )
    ...
    def reset(
        ...
        a_range = [self.a_min_init_env_nd, self.a_max_init_env_nd]  # (m)
        e_range = [self.e_min_init_env, self.e_max_init_env]
        w_range = [self.w_min_init_env_rad, self.w_max_init_env_rad]  #  (rad)
        theta_range = [
        ...
        a_target_range = [self.a_min_final_env_nd, self.a_max_final_env_nd]  # [m]
        e_target_range = [self.e_min_final_env, self.e_max_final_env]
        w_target_range = [self.w_min_final_env_rad, self.w_max_final_env_rad]  # rad
    ...
    def update_env(self, x_0, y_0, vx_0, vy_0, mass_0, a_target, e_target, w_target):
        ...
        self._state = np.array(
            [x_0, y_0, vx_0, vy_0, mass_0, x_f, y_f, vx_f, vy_f], dtype=np.float32
        )
