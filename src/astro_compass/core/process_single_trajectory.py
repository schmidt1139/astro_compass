...
from astro_compass.core.gen_Hamiltonian_transfer import gen_Hamiltonian_transfer
...
def process_single_transfer(params):
    ...
    if flag_solved and params["flag_plot_traj"] and eph_output is not None:
        ...
        fig_orb.savefig(
            os.path.join(plot_dir, ephem_filename + "_trajectory.png"), dpi=300
        )
    ...
    if flag_solved and eph_output is not None:
        eph_output.write_to_file(
            os.path.join(ephem_dir, ephem_filename + ".txt"),
            mod_vector_write_frequency=10,
        )
