...
def get_model(params, env, seed_in, model_dir):
    ...
    if params["load_model_checkpoint"]:
        print("Loading SAC model from: " + params["path_SAC_model_load"])
        model = SB3_SAC.load(
        ...
        )  # Use model_dir so SB3 creates SAC_1/ subdirectory
    else:
        ...
        if nn_arch_type == "custom":
            ...
            model = SB3_SAC(
                ...
                device="cpu",
                seed=seed_in,
                tensorboard_log=model_dir,  # Use model_dir so SB3 creates SAC_1/ subdirectory
