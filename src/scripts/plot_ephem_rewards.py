...
def plot_H_ephem_rewards():
    ...
    for i in range(len(set_ephems)):
        ...
        ax1.plot(arr_elapsed_time, arr_rewards, current_color, label=f'Ephem {i+1} Reward')
        ax2.plot(arr_elapsed_time, arr_r_tot, current_color, label=f'Ephem {i+1} Cumulative Reward')

        # Plot components
        ax3.plot(arr_elapsed_time, arr_pos_r_components, label=f'Ephem {i+1} Pos Reward', color=current_color)
        ax4.plot(arr_elapsed_time, arr_vel_r_components, label=f'Ephem {i+1} Vel Reward', color=current_color)
        ax5.plot(arr_elapsed_time, arr_throttle_r_components, label=f'Ephem {i+1} Throttle Reward', color=current_color)
        ax6.plot(arr_elapsed_time, arr_pos_r_components, label=f'Ephem {i+1} Pos Reward', color=current_color, linestyle='dashed')
        ax6.plot(arr_elapsed_time, arr_vel_r_components, label=f'Ephem {i+1} Vel Reward', color=current_color, linestyle='dotted')
        ax6.plot(arr_elapsed_time, arr_throttle_r_components, label=f'Ephem {i+1} Throttle Reward', color=current_color)
        ax6.plot(arr_elapsed_time, arr_time_r_components, label=f'Ephem {i+1} Time Multiplier', color="black", linestyle='dashdot')

        ax7.plot(arr_elapsed_time, arr_position_res, label=f'Ephem {i+1} Position Residual', color=current_color)

        ax8.plot(arr_elapsed_time, arr_time_r_components, label=f'Ephem {i+1} Time Component Reward', color=current_color)

        ax9.plot(arr_x_current, arr_y_current, label=f'Ephem {i+1} Chaser Traj', color=current_color, linestyle='dashed', linewidth=2.0)
        ax9.plot(arr_target_x_current, arr_target_y_current, label=f'Ephem {i+1} Target Traj', color=current_color)

        ax10.plot(arr_elapsed_time, arr_ttg, label=f'Ephem {i+1} Time to Go', color=current_color)

        ax11.plot(arr_elapsed_time, arr_terminated, label=f'Ephem {i+1} Terminated', color=current_color)
