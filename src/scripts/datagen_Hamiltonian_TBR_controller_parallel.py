...
def datagen_Hamiltonian_TBR_controller_parallel():
    ...
    sa_summary.append(f"Total time x cores: {elapsed_time * params['num_cores']/60.0:.2f} minutes")
    sa_summary.append(f"Trajectories per core: {trajs_per_core:.2f}")
    ...
    sa_summary.append(f"Average time per requested trajectory: {time_per_requested:.2f} seconds")
    sa_summary.append(f"Average time per requested trajectory per core: {elapsed_time/trajs_per_core:.2f} seconds")
    if total_pass > 0:
        test_log = log(f"Average time per successful trajectory: {elapsed_time/total_pass:.2f} seconds", test_log, flag_report_live)
        sa_summary.append(f"Average time per successful trajectory: {elapsed_time/total_pass:.2f} seconds")

    #write test log to file
    path_log = os.path.join(params["data_path"], "logs", "datagen_Hamiltonian_TBR_controller_log_" + time_str + ".txt")
    path_summary = os.path.join(params["data_path"], "summaries", "datagen_Hamiltonian_TBR_controller_parallel_summary_" + time_str + ".txt")
