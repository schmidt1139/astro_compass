...
def run_single_rollout(seed, params):
    flat = _flatten_config_params(params)
    rollout_env = gen_rl_environment(params if "environment" in params else flat)
    rollout_env.seed(seed)
    flat["seed_traj"] = seed
    flat["flag_report_live"] = False

    model = SB3_SAC.load(
        flat["path_SAC_model_load"],
        env=rollout_env,
        device=flat.get("eval_device", "cpu"),
        seed=seed,
        verbose=0,
    )

    return rollout_model(rollout_env, flat, model, [])


def mc_evaluate_agent(params):
    flat = _flatten_config_params(params)

    num_rollouts = flat.get("num_rollouts", 10)
    num_workers = flat.get("cores", 4)
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for f in tqdm(
            as_completed(futures), total=len(futures), desc="Parallel rollouts"
        ):
...
def plot_log_mc_results(mc_results, test_log, params):
    ...
    test_log = log("", test_log, True)
    test_log = log(
        "Mean position residual (nd): " + str(mean_position_res), test_log, True
    )
    test_log = log(
        "95 percentile position residual (nd): " + str(percentile_95_position_res),
        test_log,
        True,
    )
    test_log = log(
        "5 percentile position residual (nd): " + str(percentile_5_position_res),
        test_log,
        True,
    )
    test_log = log("", test_log, True)
    test_log = log(
        "Mean velocity residual (nd): " + str(mean_velocity_res), test_log, True
    )
    test_log = log(
        "95 percentile velocity residual (nd): " + str(percentile_95_velocity_res),
        test_log,
        True,
    )
    test_log = log(
        "5 percentile velocity residual (nd): " + str(percentile_5_velocity_res),
        test_log,
        True,
    )
    test_log = log("", test_log, True)
    ...
    plt.title(
        "Position Residuals Over Time for " + str(len(list_pos_residuals)) + " Episodes"
    )
    ...
    plt.title(
        "Velocity Residuals Over Time for " + str(len(list_vel_residuals)) + " Episodes"
    )
    plt.xlabel("Time Step")
