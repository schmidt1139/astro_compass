...
def compare_log_files_with_tolerance(path_output, path_truth, rtol=1e-5, atol=1e-8, flag_report_live=False):
    """
            ...
            if len(nums1) != len(nums2):
                ...
                for n1, n2 in zip(nums1, nums2):
                    if not np.isclose(n1, n2, rtol=rtol, atol=atol):
                        are_same = False
                        if flag_report_live:
                            print(f"Line {i+1} numerical difference: {n1} vs {n2} (diff: {abs(n1-n2)})")
                        break
