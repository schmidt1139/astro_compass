...
def copy_linear_layers(src_sequential: nn.Sequential, dst_sequential: nn.Sequential):
    ...
    assert len(src_linears) == len(dst_linears), f"Linear layer count mismatch: {len(src_linears)} vs {len(dst_linears)}"
    ...
    with torch.no_grad():
        ...
        for s, d in zip(src_linears, dst_linears):
            ...
            assert s.weight.shape == d.weight.shape, f"Weight shape mismatch {s.weight.shape} vs {d.weight.shape}"
...
def load_pretrained_nn_into_SAC(supervised_controller, sac_model):
    ...
    # SB3's actor.mu is a Linear(act_latent_dim -> act_dim)
        assert sac_model.policy.actor.mu.weight.shape == supervised_controller.fc6.weight.shape
    ...
    with torch.no_grad():
        if hasattr(sac_model.policy.actor, "log_std") and isinstance(sac_model.policy.actor.log_std, nn.Parameter):
            sac_model.policy.actor.log_std.fill_(-4.0)  # e.g., std ≈ exp(-4) ~ 0.018


def import_training_into_replay_buffer(path_training_data, 
                                       test_log,
                                       model,
                                       env,
                                       params):
    ...
    test_log = log("Using only first " + str(num_ephems_to_use) + " ephemerides", test_log, True)
    ...
    test_log = log("Number of ephemerides loaded into replay buffer: " + str(num_ephems), test_log, True)
    ...
    for eph in set_ephems:
        for i in range(eph.num_vectors):
            ...
            add_experience_to_replay_buffer(model, env_state_nd, env_action, reward, obs, done)
...
class RewardLoggerCallback(BaseCallback):
        ...
        def _on_rollout_end(self) -> None:
            ep_infos = list(self.model.ep_info_buffer) if self.model.ep_info_buffer is not None else []  # current buffer snapshot
