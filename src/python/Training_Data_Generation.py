...
def generate_nn_training_data(env, args, thread_id, output_dir="..\\..\\data\\training_ephems\\"):
...
def generate_nn_training_data_parallel(env, args):
    ...
    with ProcessPoolExecutor(max_workers=args["num_threads"]) as executor:
        ...
        for traj_idx in range(args["num_trajs"]):
            futures.append(executor.submit(generate_nn_training_data, env, args, traj_idx))
    
        for future in as_completed(futures):
            thread_id, result, filename = future.result()
            if (result):
                counter = counter + 1
            print(f"Thread {thread_id}    Success: {result}   Counter: {counter}   Ephem: {filename}")
