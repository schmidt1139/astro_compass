...
class Hamiltonian_Controller_TBT:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        allowed_kwargs = {"flag_report_live", "eps_threshold", "init_costate_guesses",
                          "root_max_iters", "timeout_per_trajectory","gamma","root_tol"}
    ...
    def shooting_iteration(self, lam_guess_shooting, eps):
        ...
        r_target_nd, r_dot_target_nd, v_theta_target_nd = self.compute_residuals_comps(x_f_dim, y_f_dim)

        residuals = np.array(
            [
                r_f_nd_p - r_target_nd,  # Final radius constraint
                vr_f_nd_p - r_dot_target_nd,  # Final radial velocity constraint
                vtheta_f_nd_p - v_theta_target_nd,  # Final tangential velocity constraint
                0.0,  # Co-state for theta shouldn't change
                lam_m_f_nd_p - lam_m_f,  # Final mass co-state should be 0
            ]
        )
    ...
    def compute_residuals_comps(self, x_f_dim, y_f_dim):
        ...
        r_target = self.a_target * (1 - self.e_target**2) / (1 + self.e_target * np.cos(theta_current))
        r_dot_target, v_theta_current, v_tot_current = compute_kep_velocities(self.a_target, self.e_target, theta_current, self.mu)
    ...
    def generate_output_ephemeris(self, ephemeris):
        ...
        for index, t in enumerate(arr_time):
            ...
            state_in = [ x_i, y_i, vx_i, vy_i, m_i, self.a_target, self.e_target, self.w_target ]
            ...
            ephemeris.add_data(
                t_i, x_i, y_i, vx_i, vy_i, m_i, 
                x_i_target, y_i_target, vx_i_target, vy_i_target, 
                ttg_i, alpha_vec[0], alpha_vec[1], u
            )
