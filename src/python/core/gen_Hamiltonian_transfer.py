...
from core.hamiltonian_control_TBT import Hamiltonian_Controller_TBT
from core.ephemeris_v3 import Ephemeris_v3
from core.exceptions import SpacecraftCollisionException, LowMassException, FirstGuessException


def gen_Hamiltonian_transfer(
    env,
    seed_traj,
    tof_scale,
    params,
    ...
    test_log = log(f"Environment type: {params['env_type']}\n", test_log, flag_report_live)
    ...
    # hamiltonian input state
    state = env.get_cartesian_state()
    a_target = info["target_a_nd"] * params["l_star"]  # target semi-major axis in km
    e_target = info["target_e_nd"]  # target eccentricity
    w_target = np.deg2rad(info["target_w_deg"])  # target argument of periapsis in rad

    h_state = [
        state[0]/1000,  # x in km
        state[1]/1000,  # y in km
        state[2]/1000,  # vx in km/s
        state[3]/1000,  # vy in km/s
        state[4],  # mass in kg
        a_target / 1000,  # target semi-major axis in km
        e_target,
        w_target
    ]
    ...
    kwargs = {
        ...
        "root_tol": params.get("root_tol", 1.0e-3),
    }
