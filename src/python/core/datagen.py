...
from core.process_single_trajectory import process_single_trajectory, process_single_transfer
...
def prepare_trajectory_tasks(params):
    ...
    for traj_num in range(params["num_trajs"]):
        ...
        if params["randomize_limits"]:
            ...
            if "theta_min_init_env_deg" in params:
                params_i["theta_min_init_env_deg"] = params[
                    "theta_min_init_env_deg"
                ][0]
                params_i["theta_max_init_env_deg"] = params[
                    "theta_max_init_env_deg"
                ][0]
...
def run_parallel_trajectory_generation(params):
    ...
    test_log = []
    test_log = log(
        "Two-Body Hamiltonian Controller", test_log, flag_report_live
    )
